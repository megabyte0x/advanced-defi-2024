...
contract UniswapV2Twap {
        ...
        returns (uint256 amountOut)
    {
        ...
        if (tokenIn == token0) {
            ...
            amountOut = uint256(
                FixedPoint.decode144(FixedPoint.mul(price0Avg, amountIn))
            );
        } else {
            amountOut = uint256(
                FixedPoint.decode144(FixedPoint.mul(price1Avg, amountIn))
            );
        }
    }
}
...
// --match-path test/uniswap-v2/exercises/UniswapV2Twap.test.sol \
// -vvv
